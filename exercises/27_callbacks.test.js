...
test('27_callbacks-1: Basic callback - invoke callback passing a number', () => {
  ...
  const processNumber = (number, callback) => callback(number)
...
test('27_callbacks-5: Process string with callback - transform to uppercase', () => {
  ...
  const processString = (transformedString, callback) => {
    callback(transformedString.toUpperCase())
  }
...
test('27_callbacks-6: Chained callbacks - multiple transformations', () => {
  ...
  const double = (num, callback) => callback(num * 2)

  const addTen = (num, callback) => callback(num + 10)
